...
const Skills = () => {
  return (
    <section
    id='skills'
    className='py-24 pb-24 px-[12vw] md:px-[20vw] font-sans'
    ...
    <div className='flex flex-wrap gap-1 lg:gap-5 py-10 justify-between'>
      {SkillsInfo.map((category)=>(
             ...
             >
              ...
              {/* Skills items - 3 per row for larger screens */}
              <Tilt
            key={category.title}
            tiltMaxAngleX={20}
            tiltMaxAngleY={20}
          ...
          >
              <div className='grid grid-cols-2 sm:grid-cols-3 gap-3 w-full'>
                {category.skills.map((skill)=>(
                  ...
                  >
                    <img src={skill.logo} alt={`${skill.name} logo`} className='w-6 h-6 sm:w-8 sm:h-8'></img>
                  </div>
                ))}
